...
当前网页已改为完全离线的本地直连，不再使用本目录服务。本目录保留供旧版网页使用。
...
- 配对二维码把 Worker 地址、空间令牌和房间密钥放入应用链接的 `#` 片段，片段不会随 HTTP 请求发送给 Worker；Cloudflare 模式由镜像端扫描操作端二维码即可加入，不设置双向扫码；页面摄像头识别失败时仍可展开备用代码；本地直连模式仍按界面完成双向 WebRTC 配对；
